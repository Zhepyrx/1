...
void main() {
  ...
  vK = uKind;
  float t = uTime;
  if (uKind == 0) { box = vec3(36.0, 26.0, 36.0); vel = vec3(1.2, -11.0, 0.5); size = 0.012; }
  ...
  if (uKind == 0) {
    up = normalize(vel);
    right = normalize(cross(up, V)) * size;
    up *= 0.55;
  } else {
  ...
  if (uKind == 0) { col = (amb * 0.9 + uLightColor * 0.05) * 0.5; alpha = 0.25; }
  ...
  float fade = smoothstep(box.x * 0.5, box.x * 0.3, length(rel.xz)) * smoothstep(0.3, 1.2, dist);
